...
**The `HealthCheckPlus` package itself must have no dependency on a third-party NuGet package.** This is a hard constraint, not a preference: features that would otherwise need a third-party library (for example, metrics instrumentation) are built on BCL primitives (`System.Diagnostics.Metrics`) specifically to preserve this. It does not mean the package has zero dependencies at all - it references `Microsoft.AspNetCore.App` (the shared ASP.NET Core framework, not a NuGet package) and its own first-party `HealthCheckPlus.Abstractions` package. `HealthCheckPlus.Abstractions`, the test project, and the `Samples/` projects are not held to this constraint.
