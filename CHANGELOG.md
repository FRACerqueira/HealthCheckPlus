...
- Fixed two internal `EventId` collisions where two unrelated log events (a publisher timeout and a publisher error; the background cycle's own unhandled-exception log and a publisher error) shared the same numeric id, making them indistinguishable when filtering logs by id alone.
