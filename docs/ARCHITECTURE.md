...
| `healthcheckplus.anomalies` | Counter | `healthcheckplus.anomaly.reason` | Internal defensive paths that were handled without failing the caller (see [Logging and anomalies](#logging-and-anomalies) below) — a rate/trend signal to alert on, complementing the Warning logs. |
