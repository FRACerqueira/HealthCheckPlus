...
`FilterReportForPublishing` excludes two kinds of cache entry from what gets hashed (`WhenReportChange`) and handed to publishers: one the consumer-supplied `HealthCheckPlusBackGroundOptions.Predicate` excludes from running here, and — separately — one that simply hasn't run even once yet (`CacheHealthCheckPlus.HasEverRun`, `Origin != HealthCheckTrigger.None`). **Point of attention**: `InitCache` seeds every check as `Healthy`/`Origin=None` before it first runs, and `CreateReport()` has no way to tell that seed apart from a genuine result — a check whose own `Delay` (`AddCheckPlus`/`AddCheckLinkTo`) outlives the background service's `Delay`+`Idle` (the README's own example values, 30s vs. 5s, trigger this on the very first cycle) would otherwise be published as a real Healthy observation before it was ever actually checked.
