...
**Point of attention**: once `TryBeginRun` marks a check `Running`, only `ApplyBatchResults` (via `Update`/`ReleaseRunning`) can clear it again - so every step between the two, for the whole batch marked in one `BuildDueRegistrations` call, must guarantee that release happens even on failure, or the check stays `Running` forever (excluded from every future schedule, with no recovery short of a process restart). `CheckHealthPlusAsync`/`BackGroudCheckHealthPlusAsync` wrap the logging call and `StartBatch` in their own try/catch for exactly this reason - a broken `ILogger` sink used to throw from completely outside any release path. `BuildDueRegistrations`'s own loop has the same guard for a registration marked `Running` earlier in the same loop, in case a later one's policy resolution throws.
